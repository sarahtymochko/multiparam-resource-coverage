        ...
        for j, TC2 in enumerate(all_TCs):
            if i!=j:
                ...
                if TC2_max_rep.issubset(TC1_max_rep):
                    ...
                    TC1.C_reps = merge_dicts(TC1.C_reps, TC2.C_reps)
...
def merge_dicts(dict1, dict2):
        ...
        for key in all_keys:
            if key in dict1 and key in dict2:
                # Key exists in both - take the union
                merged[key] = set(dict1[key]) | set(dict2[key])
            elif key in dict1:
...
def postprocess_merge_similar(all_TCs, threshold=0.9):
    ...
    for i, TC1 in enumerate(all_TCs): #enumerate(all_TCs):
        ...
        for j, TC2 in enumerate(all_TCs):
            if i!=j:
                ...
                # intersection_size = len(TC2_max_rep.intersection(TC1_max_rep))
                # union_size = len(TC2_max_rep.union(TC1_max_rep))

                if max(amt_1_in_2, amt_2_in_1) >= threshold:
                    print(f'Merging {i} and {j}')
                    new_TC1 = copy.deepcopy(TC1)
                    new_TC1.C_reps = merge_dicts(TC1.C_reps, TC2.C_reps)
                    new_TC_list.append(new_TC_list)
    ...
    return [all_TCs[i] for i in range(len(all_TCs)) if i not in delete_inds] + new_TC_list
